...
describe('FilesAgent', () => {
  ...
  it('should gather information from multiple files', async () => {
    vi.mocked(readFile).mockResolvedValue('File content');
    vi.mocked(stat).mockResolvedValue({ isDirectory: () => false } as any);
    vi.mocked(glob).mockImplementation(() => Promise.resolve(['file1.txt', 'file2.txt']));
  ...
  it('should handle directories and gather information from files within', async () => {
    ...
    vi.mocked(glob).mockImplementation((pattern, options) => 
      Promise.resolve(mockFiles.filter(file => !mockDefaultExclusions.includes(file))));
  ...
  it('should handle exclude patterns correctly', async () => {
    const mockFiles = ['file1.txt', 'file2.txt', '.git/HEAD', 'node_modules/package.json'];
    vi.mocked(readFile).mockResolvedValue('File content');
    vi.mocked(stat).mockImplementation((path) => Promise.reject(new Error(`No such file or directory: ${path}`)) as any);
    vi.mocked(glob).mockImplementation((pattern, options) => 
      Promise.resolve(mockFiles.filter(file => !mockDefaultExclusions.includes(file))));
    ...
    const expected = mockFiles
      .filter(file => !mockDefaultExclusions.some(exclusion => file.includes(exclusion)))
      .map(file => ({
        tag: 'file',
        attrs: { name: file },
        content: 'File content',
